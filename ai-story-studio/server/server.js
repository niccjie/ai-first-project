...
function createApp({ env = process.env, fetchImpl = fetch, timeoutMs = 55000 } = {}) {
  ...
  app.get('/api/health', (_req, res) => res.json({ service: 'ai-creator-studio', configured: Boolean(env.OPENAI_API_KEY?.trim() && env.OPENAI_MODEL?.trim()) }));
  ...
  app.post('/api/create-story', async (req, res) => {
    ...
    if (!env.OPENAI_API_KEY?.trim() || !env.OPENAI_MODEL?.trim()) return res.status(503).json({ error: '请在 server/.env 配置 OPENAI_API_KEY 和 OPENAI_MODEL，然后重启服务。' });
    ...
    try {
      const upstream = await fetchImpl('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          model: env.OPENAI_MODEL.trim(), store: false, max_output_tokens: 6000,
      ...
      if (!upstream.ok) {
        ...
        return res.status(code).json({ error: code === 401 ? 'OpenAI 密钥验证失败，请检查服务端配置。' : code === 429 ? 'OpenAI 配额或速率受限，请稍后重试。' : 'OpenAI 服务请求失败，请检查模型配置或稍后重试。' });
      }
...
if (require.main === module) {
  ...
  const server = createApp().listen(port, '127.0.0.1', () => {
    ...
    if (!process.env.OPENAI_API_KEY?.trim() || !process.env.OPENAI_MODEL?.trim()) console.log('请填写 server/.env；未配置时 Demo 可用，真实生成返回 503。');
  });
